  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
        <div>
          <h2 class="text-xl sm:text-2xl font-bold text-gray-900">{{ $t('admin.userManagement.title') }}</h2>
          <p class="text-sm sm:text-base text-gray-600 mt-1">{{ $t('admin.userManagement.description') }}</p>
        </div>
        <button
          ...
          <table class="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-if="users.length === 0">
                <td colspan="5" class="px-4 sm:px-6 py-8 sm:py-12 text-center text-gray-500">
                  ...
                  <p class="text-base sm:text-lg font-medium">{{ $t('admin.userManagement.noUsers') }}</p>
                </td>
              </tr>
              <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-white font-semibold text-sm sm:text-base">
                      {{ user.name.charAt(0).toUpperCase() }}
                    </div>
                    <div class="ml-3 sm:ml-4">
                      <div class="text-xs sm:text-sm font-medium text-gray-900 flex items-center gap-2">
                        {{ user.name }}
    ...
    >
      ...
      >
        ...
        <form @submit.prevent="saveUser" class="p-4 sm:p-6 space-y-4">
          <!-- Name -->
          <div>
    ...
    >
      ...
      >
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            <h3 class="text-base sm:text-lg font-semibold text-gray-900">{{ $t('admin.userManagement.deleteUser') }}</h3>
